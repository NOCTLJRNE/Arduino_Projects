...
//Adafruit_BicolorMatrix matrix = Adafruit_GFX(8, 16);
void setup() {
  uint8_t i, j, nBins, binNum, *data;

  memset(peak, 0, sizeof(peak));
  memset(col , 0, sizeof(col));
  EsploraTFT.begin();//TFTscreen.begin();
  EsploraTFT.background(250, 16, 200);//TFTscreen.background(50, 50, 50);
  Serial.begin(9600);
  for(i=0; i<8; i++) {
    minLvlAvg[i] = 0;
    maxLvlAvg[i] = 512;
...
void loop() {
  ...
  for(x=0; x<8; x++) 
  {
    ...
    if(peak[x] <= 0) { // Empty column?
      ...
      //led_value += (led_value << 3) + c;
 
    }
    led_value += (led_value << 4) + c;    
  ...
  matrix.writeDisplay();
    ...
    {
      //int drawHeight = map(matrix.displaybuffer[xPos], 0, 1023, 0, TFTscreen.height());
      int drawHeight = map(matrix.displaybuffer[xPos], 0, 1023, 0, EsploraTFT.width());//int drawHeight = map(matrix.displaybuffer[xPos], 0, 1023, 0, TFTscreen.width());
      for (int j = 48; j < 52; j++)
      {
      EsploraTFT.stroke(0, 0, 255);//TFTscreen.stroke(0, 0, 255);
      //TFTscreen.line(xPos*4+j, 128 - drawHeight, xPos*4+j, 128);
      EsploraTFT.line(160 - drawHeight, xPos*4+j, 160, xPos*4+j);//TFTscreen.line(160 - drawHeight, xPos*4+j, 160, xPos*4+j);
      }
    }
 //if (Serial.available())
 //{   
  ...
  for (x=0; x<7; x++)
 { 
  Serial.print(matrix.displaybuffer[x]);
  Serial.print(" value:");
 } 
 Serial.println(matrix.displaybuffer[7]);
 */
 //}
