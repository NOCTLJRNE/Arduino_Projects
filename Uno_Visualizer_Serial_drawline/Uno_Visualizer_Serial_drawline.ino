...
volatile int16_t sample_global = 0;
...
void loop() {
  ...
  //}
    ...
    for(xPos = 0; xPos<7; xPos++) 
    {
      ...
      int drawHeight = map(matrix.displaybuffer[xPos], 0, 1023, 0, TFTscreen.width());
      for (int j = 48; j < 52; j++)
      {
      TFTscreen.stroke(0, 0, 255);
      //TFTscreen.line(xPos*4+j, 128 - drawHeight, xPos*4+j, 128);
      TFTscreen.line(160 - drawHeight, xPos*4+j, 160, xPos*4+j);
      }
    }
 //if (Serial.available())
 //{   
