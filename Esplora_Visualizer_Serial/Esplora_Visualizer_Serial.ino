...
void loop() {
  ...
  matrix.writeDisplay();
    ...
    {
      //int drawHeight = map(matrix.displaybuffer[xPos], 0, 1023, 0, EsploraTFT.height());
      //int drawHeight = map(matrix.displaybuffer[xPos], 0, 1023, 0, EsploraTFT.width());
      for (int j = 48; j < 52; j++)
      {
      //EsploraTFT.stroke(0, 0, 255);
      //EsploraTFT.line(xPos*4+j, 128 - drawHeight, xPos*4+j, 128);
      //EsploraTFT.line(160 - drawHeight, xPos*4+j, 160, xPos*4+j);
      }
    }
 //if (Serial.available())
 //{   
